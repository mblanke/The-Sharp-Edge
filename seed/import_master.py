...
def parse_ingredient(line: str, section: str | None = None) -> dict:
    """One master-file bullet → {amount, unit, name[, section]}. amount 0 = to taste/unscaled."""
    ...
    if re.search(r"to taste", row["name"], re.I) and row["unit"] == "":
        pass  # names like "salt, to taste" already read right; amount stays as parsed
    if section:
        row["section"] = section
    return row
